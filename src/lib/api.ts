...
export async function fetchQuestionsByFormId(formId: string) {
  const res = await fetch(`${getApiUrl()}/questions?form_id=${formId}`)
...
export async function fetchQuestionsBySectionId(sectionId: string) {
  try {
    const res = await fetch(`${getApiUrl()}/questions?section_id=${sectionId}`);
...
export async function getSections(formId: string) {
  const res = await fetch(`${getApiUrl()}/forms/${formId}/sections/ordered`)
...
export async function deleteQuestion(id: string): Promise<void> {
  try {
    console.log('NEXT_PUBLIC_API_URL:', process.env.NEXT_PUBLIC_API_URL);
    const url = `${getApiUrl()}/questions/${id}`;
...
export async function copyQuestion(questionId: string): Promise<any> {
  try {
    ...
    const url = `${getApiUrl()}/questions/${questionId}/copy`;
...
export async function updateQuestion(id: string, data: Partial<{
  ...
  const res = await fetch(`${getApiUrl()}/questions/${id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function getChoicesByQuestionId(questionId: string) {
  try {
    const res = await fetch(`${getApiUrl()}/choices/${questionId}/ordered`)
...
export async function uploadDocument({
  ...
  const res = await fetch(`${(getApiUrl() ?? '').replace(/\/api$/, '')}/api/documents/upload`, {
    method: 'POST',
    body: formData,
    credentials: 'include'
  })
...
export async function submitAnswers({
  ...
  await fetch(`${(getApiUrl() ?? '').replace(/\/api$/, '')}/sanctum/csrf-cookie`, {
    credentials: 'include'
  })
...
export async function uploadTempDocument({
  ...
  const res = await fetch(`${getApiUrl()}/upload-temp`, {
    method: 'POST',
    body: formData,
  });
...
export async function getTempUploads(session_id: string) {
  const res = await fetch(`${getApiUrl()}/temp-uploads?session_id=${encodeURIComponent(session_id)}`);
  if (!res.ok) {
    const text = await res.text();
...
export async function deleteTempUpload(id: string) {
  const res = await fetch(`${getApiUrl()}/temp-uploads/${id}`, {
    method: 'DELETE',
  });
  if (!res.ok) {
    const text = await res.text();
...
export async function saveFormDraft({ nik, form_id, draft_data }: { nik: string, form_id: string, draft_data: any }) {
  const res = await fetch(`${getApiUrl()}/form-draft`, {
    method: 'POST',
...
export async function getFormDraft({ nik, form_id }: { nik: string, form_id: string }) {
  const res = await fetch(`${getApiUrl()}/form-draft?nik=${nik}&form_id=${form_id}`);
...
export async function getFormSubmissions(formId: string) {
  const res = await fetch(`${getApiUrl()}/forms/${formId}/submissions`);
  if (!res.ok) {
    const error = await res.json();
...
export async function getFormAnswers(formId: string, nik: string, sequence: string) {
  const res = await fetch(`${getApiUrl()}/forms/${formId}/answers/${nik}?sequence=${sequence}`);
  if (!res.ok) {
    const error = await res.json();
...
export async function createTemplateQuestions(formId: string, templateQuestions: any[]) {
  ...
  try {
    for (const template of templateQuestions) {
      ...
      const apiUrl = getApiUrl();
      console.log('Making request to:', `${apiUrl}/questions`);
      ...
      if (template.choices && template.choices.length > 0) {
        ...
        const choiceRes = await fetch(`${getApiUrl()}/questions/${createdQuestion.questionId || createdQuestion.id}/choices/map`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
